...
def Classification_metrics(clf_results_folder,Antibiotic_data_path,kmer_path,feature_dir_path):
    #Add the name of the data set
    name_dataset = ""
    method = "model"
    #Add the folder where the results must be saved
    results_folder = "step2/4filt-2-neighbour_res"
    
    # Load AMR profile:
    antibiotic_df = pd.read_csv(Antibiotic_data_path, header = [0])
    delimiter = '\t'
    ...
    for name_antibiotic in antibiotic_df.columns[1:]:
        try: 
            ...
            if count_class[0] < 5 or count_class[1] < 5:
                continue
            features_df = pd.read_csv(feature_dir_path+"/_"+name_antibiotic+"_"+method+"_pvalue.csv", header = None)
            ...
            update_progress(0)
            for n, line in enumerate(open(kmer_path,'r')):
            ...
            for i in range(NUM_TRIALS):
                ...
                for name, clf in zip(names, classifiers):
                    ...
                    smote = SMOTE(sampling_strategy='auto', random_state=i, k_neighbors=2)  # Adjust the k_neighbors parameter
            ...
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_auc.csv", scores_auc, delimiter=",")
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_acc.csv", scores_acc, delimiter=",")
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_sens.csv", scores_sens, delimiter=",")
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_spec.csv", scores_spec, delimiter=",")
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_prec.csv", scores_prec, delimiter=",")
            np.savetxt(clf_results_folder+"/SMOTE_"+method+"_"+name_dataset+"_"+name_antibiotic+"_kappa.csv", scores_kappa, delimiter=",")
            results_df.to_csv(clf_results_folder+"/SMOTE_"+method+"_"+"results_"+name_dataset+"_"+name_antibiotic+".csv")
